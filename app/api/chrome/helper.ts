...
export const isRunningInAwsLambda = () => {
	if (process.env['AWS_EXECUTION_ENV'] && /^AWS_Lambda_nodejs/.test(process.env['AWS_EXECUTION_ENV']) === true) {
		return true
	} else if (process.env['AWS_LAMBDA_JS_RUNTIME'] && /^nodejs/.test(process.env['AWS_LAMBDA_JS_RUNTIME']) === true) {
		return true
	}
	return false
}
